...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($one === "W2W") 
    {
        ...
        // $pdf->Cell(0, 10, 'Particulars: '. $one. '/'. $two, 0, 1);
    ...
    elseif($one === "W2B")
    {
        ...
        $pdf->SetFont('times', '', 10);
        // Add transaction details
        $pdf->Cell(0, 10, 'Name: ' . ($_SESSION["first_name"] . ' ' . $_SESSION["last_name"]), 0, 1);
        $pdf->Cell(0, 10, 'Wallet Address: ' . $senderWalletAddress, 0, 1);
        $pdf->Cell(0, 10, 'Bank Name: ' . $BankName, 0, 1);
        $pdf->Cell(0, 10, 'Bank Account Number: ' . $accountNo, 0, 1);
        $pdf->Cell(0, 10, 'Amount: INR ' . $amount, 0, 1);
        // $pdf->Cell(0, 10, 'Particulars: '. $one. '/'. $two, 0, 1);
